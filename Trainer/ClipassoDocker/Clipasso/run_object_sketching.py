...
print(f"In the Run_object_sketching this is the absolute path {abs_path}")

target = args.target_file if os.path.isabs(args.target_file) else os.path.join(abs_path, "target_images", args.target_file)
...
def run(seed, wandb_name):
    print("running")
    exit_code = sp.run(["python3", "painterly_rendering.py", target,
...
if not sorted_final:
    print(f"⚠️ No sketches found in {output_dir}. Skipping file.")
else:
    print("whatttt")
